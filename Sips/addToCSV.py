...
for index, row in df.iterrows():
    ...
    for i in range(0, len(sections)-1, 2):
        ...
        drink_type = sections[i].replace('$', '').strip().split("Half-Priced Appetizers")[0]
        ...
        menu_items = [item for item in menu_items if "all beer" not in item.lower() and "all wine" not in item.lower()]

        
        # Exclude "rotating draft"
        menu_items = [item for item in menu_items if "rotating draft" and "15% off dinner!" not in item.lower()]
